    ...
    <div class="card">
        ...
        <div style="display:grid; grid-template-columns:1fr 1fr; gap:16px;">
            ...
            <div>
                ...
                <span style="color:#0C3547; font-weight:600;">{{ $site->frequency_min }} min</span>
            </div>
            <div>
                <div style="font-size:11px; color:#6B7280; margin-bottom:4px;">Seuil réponse</div>
                <span style="color:#0C3547; font-weight:600;">{{ $site->response_threshold_ms }} ms</span>
            </div>
            <div>
...
{{-- Calcul données 30 jours AVANT le script --}}
@php
    $uptimeData = [];
    for ($i = 29; $i >= 0; $i--) {
        ...
        if ($total > 0) {
            $uptimeData[] = ['x' => $label, 'y' => round($up / $total * 100, 1)];
        }
    }
@endphp
...
<div class="table-wrapper" style="margin-bottom:24px;">
    <div class="table-header">
        <div style="font-size:15px; font-weight:600; color:#0C3547;">🔍 Dernières vérifications</div>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                <td style="color:#64748B; font-size:12px;">{{ $v->checked_at->format('d/m/Y H:i:s') }}</td>
                <td>
                ...
                <td>
                    <span style="color:{{ $v->response_time_ms > 2000 ? '#EF4444' : ($v->response_time_ms > 1000 ? '#D97706' : '#10B981') }}; font-weight:600;">
...
<div class="table-wrapper">
    <div class="table-header">
        <div style="font-size:15px; font-weight:600; color:#0C3547;">⚠️ Incidents récents</div>
        <span class="badge badge-red">{{ $incidents->count() }} incidents</span>
    </div>
    <table>
        <thead>
            <tr>
                <th>Type</th><th>Début</th><th>Résolu</th><th>Durée</th><th>Statut</th>
            </tr>
        </thead>
        <tbody>
            ...
            <tr>
                ...
                <td style="color:#9CA3AF; font-size:12px;">{{ $incident->started_at->format('d/m/Y H:i') }}</td>
                <td style="color:#9CA3AF; font-size:12px;">{{ $incident->resolved_at ? $incident->resolved_at->format('d/m/Y H:i') : '—' }}</td>
                <td style="color:#F59E0B; font-weight:600;">{{ $incident->duration_min ? $incident->duration_min . ' min' : 'En cours' }}</td>
                <td>
...
<div class="card" style="margin-top:16px;">
    ...
    <div style="display:grid; grid-template-columns:repeat(4,1fr); gap:16px;">
        <div style="text-align:center; padding:16px; background:#F0F9FF; border-radius:10px;">
            <div style="font-size:16px; font-weight:800; color:#0C3547;">{{ $site->domain_registrar ?? '—' }}</div>
        ...
        <div style="text-align:center; padding:16px; background:#F0F9FF; border-radius:10px;">
            ...
            <div style="font-size:16px; font-weight:800;
                ...
                {{ $site->domain_expires_at ? \Carbon\Carbon::parse($site->domain_expires_at)->format('d/m/Y') : '—' }}
            </div>
        ...
        <div style="text-align:center; padding:16px; background:#F0F9FF; border-radius:10px;">
            <div style="font-size:16px; font-weight:800; color:#0C3547;">
                {{ $site->domain_created_at ? \Carbon\Carbon::parse($site->domain_created_at)->format('d/m/Y') : '—' }}
            </div>
...
new Chart(document.getElementById('responseChart'), {
    ...
    data: {
        datasets: [{
            ...
            data: verifications,
            borderColor: '#1697C2',
            backgroundColor: 'rgba(22,151,194,0.1)',
...
// Graphique 30 jours
@if(count($uptimeData) > 0)
const uptimeData = @json(array_values($uptimeData));
